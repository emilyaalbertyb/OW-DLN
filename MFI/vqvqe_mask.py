...
class VQVAE(nn.Module):
    ...
    def forward(self, input, mask):
        # input_with_mask = torch.cat([input, mask], dim=1)
        input_with_mask = input * mask
        quant_t, quant_b, diff, _, _ = self.encode(input_with_mask)
        dec = self.decode(quant_t, quant_b)
...
#
    ...
    def encode(self, input):
        ...
        #对掩码区域添加噪声
        # quant_b = self.apply_noise_to_mask(quant_b, input[:,-1:,:,:])

        return quant_t, quant_b, diff_t + diff_b, id_t, id_b
    def apply_noise_to_mask(self, quant_b, mask):
        noise = torch.randn_like(quant_b) * mask
        return quant_b + noise
