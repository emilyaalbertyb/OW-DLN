...
class SimpleRectangleMaskGenerator:
    ...
    def __call__(self, img):
        ...
        # 生成矩形掩码
        mask[y1:y1 + rect_height, x1:x1 + rect_width] = 1

        return [mask]  # 返回一个包含单个掩码的列表
...
def process_images(src_images, indir, outdir, config):
    if config.generator_kind == 'segmentation':
        ...
        # mask_generator = MakeManyMasksWrapper(SimpleRectangleMaskGenerator())
    else:
